...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // ──────────────────────────────────────────────────────
    var dir = CONFIG.leadership.director;
    var pri = CONFIG.leadership.principal;

    setText("[data-director-name]",          dir.name);
    setText("[data-director-designation]",   dir.designation);
    setText("[data-director-qualification]", dir.qualification);
    setText("[data-director-experience]",    dir.experience);
    setText("[data-director-message]",       dir.message);
    setAttr("[data-director-photo]", "src",  dir.photo);
    setAttr("[data-director-photo]", "alt",  dir.name);

    setText("[data-principal-name]",          pri.name);
    setText("[data-principal-designation]",   pri.designation);
    setText("[data-principal-qualification]", pri.qualification);
    setText("[data-principal-experience]",    pri.experience);
    setText("[data-principal-message]",       pri.message);
    setAttr("[data-principal-photo]", "src",  pri.photo);
    setAttr("[data-principal-photo]", "alt",  pri.name);


    // ──────────────────────────────────────────────────────
    //  17. NOTICES  (any page with [data-notices])
    // ──────────────────────────────────────────────────────
    var noticeContainer = document.querySelector("[data-notices]");
    if (noticeContainer) {
        noticeContainer.innerHTML = CONFIG.notices.map(function(n) {
            return (
                '<div class="notice-item' + (n.isImportant ? ' important' : '') + '">' +
                    '<div class="notice-date">' + sanitize(n.date) + '</div>' +
                    '<div class="notice-category">' + sanitize(n.category) + '</div>' +
                    '<h4>' + sanitize(n.title) + '</h4>' +
                    '<p>' + sanitize(n.description) + '</p>' +
                    (n.pdfUrl && n.pdfUrl !== "#"
                        ? '<a href="' + n.pdfUrl + '" class="notice-download" download><i class="fas fa-download"></i> Download</a>'
                        : '') +
                '</div>'
            );
        }).join("");
